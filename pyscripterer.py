class BaseScript(object):
    ...
    def get_param(self, param_name):
        """Get the param by param_name"""
        for param in self.helpers.analyzeRequest(self.messageInfo).getParameters():
    ...
    def get_response(self):
        ...
        return self.helpers.bytesToString(response_bytes)
    ...
    def new_base64(self,text):
        ...
        if len(text) > 5:
            ext = text[2:3] + text[-5:-4]
        
        return text[:1] + ext + text[1:]
    ...
    def add_request_headers(self, header_names):
        """add a list of headers from the current request."""
        ...
        new_request = self.helpers.buildHttpMessage(headers, body)
        self.messageInfo.setRequest(new_request)
        self._debug('Headers add: {}'.format(', '.join(header_names)))
    

    def replace_request_param(self, param_name, value):
        ...
        new_request = self.helpers.removeParameter(request, param)
        new_param = self.helpers.buildParameter(param_name, value, param.getType())
        new_request = self.helpers.addParameter(new_request, new_param)
        self.messageInfo.setRequest(new_request)
        self._debug('Param change: {}'.format(param_name))
    ...
    def replace_response_data(self, data):
        """Replace the body to the data"""
        if not self.is_in_context(context='response'): return


        response_bytes = self.messageInfo.getResponse()
        headers = self.helpers.analyzeResponse(response_bytes).getHeaders()
        new_response = self.helpers.buildHttpMessage(headers, self.helpers.stringToBytes(data))

        self.messageInfo.setResponse(new_response)
        self._debug('Response replaced !')
